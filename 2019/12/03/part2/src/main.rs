...
fn source_to_map(source: &str) -> Vec<Point> {
    #[derive(Debug)]
    struct Acc {
        current: Point,
        points: Vec<Point>,
    ...
    impl Acc {
        fn up(&mut self, steps: usize) {
            let ending = self.current.y + steps as i64;
        ...
        fn down(&mut self, steps: usize) {
            let start = self.current.y - steps as i64;
            ...
            collector.reverse();
            self.points.append(&mut collector);
            self.current = Point {
                x: self.current.x,
                y: start,
            };
        ...
        fn right(&mut self, steps: usize) {
            let ending = self.current.x + steps as i64;
        ...
        fn left(&mut self, steps: usize) {
            let start = self.current.x - steps as i64;
            let mut collector = Vec::new();
    ...
    source
        .split(',')
        .fold(accumulator, |mut acc, instruction| {
